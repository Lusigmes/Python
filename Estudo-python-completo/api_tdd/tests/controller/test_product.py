...
async def test_controller_get_should_return_not_found(client, products_url):
    response = await client.get(f"{products_url}6f123456-18ba-45f5-1234-333e222a111c")

    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {
        "detail": "Product not found with ID: 6f123456-18ba-45f5-1234-333e222a111c"
    }
...
async def test_controller_update_should_return_success(
    ...
    response = await client.update(
        f"{products_url}{product_inserted.id}", json={"valor": "5.500"}
    )
    content = response.json()
...
async def test_controller_delete_should_return_not_found(client, products_url):
    response = await client.delete(
        f"{products_url}6f123456-18ba-45f5-1234-333e222a111c"
    )
